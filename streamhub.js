/* ============================================================================
   PROYECTO: StreamHub - Gestión de contenido y usuarios en MongoDB
   Archivo: streamhub.js
   Descripción: Script ejecutable en mongosh (MongoDB Shell) o MongoDB Compass
                (pestaña "mongosh") que cubre las 5 tareas de la historia de
                usuario: diseño, inserción, consultas, updates/deletes,
                índices y agregaciones.

   Cómo ejecutarlo:
     1. Abrir mongosh:            mongosh
     2. Cargar el script:         load("streamhub.js")
        (ajustar la ruta según donde se descomprima el .zip)
   ============================================================================ */
...
/* ============================================================================
   TASK 1: DISEÑO DE COLECCIONES Y DOCUMENTOS
   ============================================================================
   Colecciones definidas:

   1) usuarios
      - Representa a cada usuario de la plataforma.
      - Incluye un arreglo embebido "historial" (contenidos vistos) porque
        se consulta casi siempre junto con el usuario (patrón "embedding"
        para datos 1:N que no crecen indefinidamente).
      - Estructura:
        {
   ...
   2) contenidos
      - Unifica películas y series bajo un solo esquema con el campo "tipo"
        ("pelicula" | "serie") para simplificar búsquedas transversales
        (p. ej. buscar por género sin importar el tipo).
      - Campos variables según tipo (duracionMinutos vs temporadas) para
        aprovechar la flexibilidad de esquema de MongoDB (semiestructurado).
      - Estructura:
        {
          _id, titulo, tipo, genero: [String], anioLanzamiento,
          duracionMinutos (solo películas), temporadas (solo series),
   ...
   3) valoraciones
      - Colección independiente (referenced pattern) porque crece
        indefinidamente (N valoraciones por usuario y por contenido) y se
        necesita agregarlas/filtrarlas de forma independiente.
      - Estructura:
        { _id, usuarioId, contenidoId, puntuacion (1-5), comentario, fecha }

   4) listas
      - Listas personalizadas ("Mi lista", "Favoritos") creadas por usuarios.
      - Estructura:
        { _id, usuarioId, nombreLista, contenidos: [contenidoId], fechaCreacion }

   Relaciones:
   - usuarios.historial[].contenidoId  -> referencia a contenidos._id
   - valoraciones.usuarioId            -> referencia a usuarios._id
   - valoraciones.contenidoId          -> referencia a contenidos._id
   - listas.usuarioId                  -> referencia a usuarios._id
   - listas.contenidos[]               -> referencia a contenidos._id
   ============================================================================ */
...
// ---- 2.3 contenidos: insertMany (películas y series con reseñas embebidas
//       de ejemplo dentro de "resenasDestacadas" para mostrar documentos
//       anidados, además de las valoraciones normalizadas en su colección) ----
...
/* ============================================================================
   TASK 6: AGREGACIONES (PIPELINES) — MÉTRICAS Y REPORTES
   (Se entregan 4 pipelines, superando el mínimo de 2 solicitado)
   ============================================================================ */

// ---- 6.1 Calificación promedio real y número de reseñas por contenido,
//      ordenado de mayor a menor calificación (usa $match, $group, $sort,
//      $project) ----
...
// ---- 6.2 Cantidad de contenidos por género (usa $unwind porque "genero"
//      es un arreglo, $group y $sort) ----
